...
export default function Sidebar({ onAdd, onLogout }) {
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem("currentUser"); // hapus session
    onLogout?.(); // update state App.jsx
  };
          ...
          {hoverUser && (
            <div
              className="absolute left-16 top-1/2 -translate-y-1/2 w-60 p-4 rounded-2xl
                          backdrop-blur-lg shadow-xl z-[99]"
              style={{ backgroundImage: "linear-gradient(to bottom right, #340e57, #9b7c32)" }}
            >
